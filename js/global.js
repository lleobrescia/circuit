$(function () {
  ...
  $(document).on("click", ".header__toggle", function () {
    ...
    if ($(this).hasClass('header__toggle--open')) {
      ...
      $('.menu-top').removeClass('menu-top--open');
    } else {
      ...
      $.ajax({
        ...
        if (podeComprar == false) {
          ...
          $.colorbox({
            width: "30%",
            inline: true,
            href: "#messageQtdDenied"
          });
        } else {
          console.log('quantidades corretas');
          javascript: if (typeof validarPrePost == 'function') {
            if (validarPrePost()) {
              AjaxRequestDispatcher('pnlCarrinho_cart|btnCarrinhoContinuar');
            }
          } else {
            AjaxRequestDispatcher('pnlCarrinho_cart|btnCarrinhoContinuar');
          }
